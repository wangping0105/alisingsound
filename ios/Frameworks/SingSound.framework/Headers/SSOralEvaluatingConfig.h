...
typedef NS_ENUM(NSInteger, OralType) {
    ...
    OralTypePoet,                                   //中文诗歌        (cn.poet.score)
    
    OralTypeCnRec,                                  //中文识别        (cn.sent.rec)
    OralTypeEnRec,                                  //英文识别        (en.sent.rec)
};
...
/**
 传输音频是否压缩，默认压缩 defaults is YES,
 */
@property (nonatomic, assign) BOOL compress;

/**
 是否输出压缩后的音频数据，默认不输出 defaults is NO,
 */
@property (nonatomic, assign) BOOL saveAudio;
...
/**
 评分松紧度，可传 1，2，3，4。1 表示严格，2 表示宽松，3 表示非常严格，4 非常宽松。和rateScale不能同时传
 */
 ...
 */
@property (nonatomic, assign) BOOL removeHeader;
